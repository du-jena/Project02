...
Wings.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

Ground.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "wings") {
    var level = this;
    setTimeout(function(){
      level.status = null, gravity = 25}, 1000);
  ...
  if (type == "ground") {
    var level = this;
    setTimeout(function(){
      level.status = null, gravity = 50}, 1000);
      this.actors = this.actors.filter(function(other) {
        return other != actor;
      });
  }
  ...
  } else if (type == "coin" || type == "fire") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any coins left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "coin" || actor.type == "fire"
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
